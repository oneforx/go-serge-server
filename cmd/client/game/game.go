...
func (g *SquirrelGame) Update() error {
	...
	if entities := g.World.GetEntitiesPossessedBy(*g.Token); len(entities) > 0 {
		player := *entities[0]
		playerPosition, ok := player.GetComponent(goecs.Identifier{Namespace: "oneforx", Path: "position"}).Data.(map[string]interface{})
		if !ok {
	...
	if entitiesWeOwn := g.World.GetEntitiesPossessedBy(*g.Token); len(entitiesWeOwn) > 0 {
		...
		bytes, err := goserge.MessageToBytes(messages.CS_ORIENTATION(mx, my))
		if err != nil {
		...
		if g.PressedKeys = inpututil.AppendPressedKeys(g.PressedKeys[:0]); len(g.PressedKeys) != 0 {
			for _, k := range g.PressedKeys {
				switch k.String() {
				case "D":
					move_right_message, err := goserge.MessageToBytes(goserge.Message{
						MessageType: "MOVE",
						Data:        "RIGHT",
					})
					if err != nil {
				...
				case "A":
					move_left_message, err := goserge.MessageToBytes(goserge.Message{
						MessageType: "MOVE",
						Data:        "LEFT",
					})
					if err != nil {
		...
		if leftButtonPressed {
			...
			bytes, err := goserge.MessageToBytes(messages.CS_SHOOT())
			if err != nil {
...
func (g *SquirrelGame) Draw(screen *ebiten.Image) {
	...
	if len(g.World.GetEntities()) > 0 {
		for _, entity := range g.World.GetEntities() {
			...
			positionComponent := entityLocalisation.GetComponent(goecs.Identifier{Namespace: "oneforx", Path: "position"})
