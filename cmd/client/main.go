...
import (
	...
	"github.com/oneforx/go-serge-server/cmd/client/game"
...
func main() {
	...
	go func() {
		...
		if squirrelGame.Token != nil {
			bytes, err := goserge.MessageToBytes(messages.CS_CONNECT_TOKEN(squirrelGame.Token.String()))
			if err != nil {
		...
		for {
			...
			case "CREATE_WORLD":
				type CustomMessage struct {
					...
					Data        []goecs.EntityNoCycle `json:"data"`
				}
			...
			case "CONNECT_SUCCESS":
				...
				if err == nil {
					...
					bytesForUdp, err := goserge.MessageToBytes(messages.CS_CONNECT_TOKEN(squirrelGame.Token.String()))
					if err != nil {
						log.Println("Could not parse message to bytes")
					}
			...
			case "CREATE_ENTITY":
				type CustomMessage struct {
					MessageType string              `json:"message"`
					Data        goecs.EntityNoCycle `json:"data"`
					Target      goserge.TargetType
					NetMode     goserge.NET_MODE
				}
				...
				sync_mutex.Lock()
				squirrelGame.World.AddEntity(goecs.EntityNoCycleToEntity(&squirrelGame.World, customMessage.Data))
			...
			case "PING":
				...
				bytes, err := goserge.MessageToBytes(messages.CS_PONG())
				if err != nil {
					continue
				}
		...
		for i := 0; i < 10; i++ {
			go func() {
				for {
					...
					message, err := goserge.BytesToMessage(buf[:n])
					if err != nil {
						continue
					}
	...
	go func() {
		...
		for {
			...
			if scannerText[0] == "DISCONNECT" {
				var message goserge.Message = goserge.Message{
				...
				bytes, err := goserge.MessageToBytes(messages.CS_CONNECT_WORLD())
				if err != nil {
				...
				bytes, err := goserge.MessageToBytes(messages.CS_DISCONNECT_WORLD())
				if err != nil {
...
func SendTCPMessage(tcpConnexionPointer *net.Conn, message goserge.Message) error {
...
func SendUDPMessage(udpConnexionPointer *net.UDPConn, message goserge.Message) error {
	messageBytes, err := json.Marshal(message)
	if err != nil {
		return fmt.Errorf("could not marshal message from user keyboard entry")
	}
